...
fn main() {
    ...
    .add_systems(Update, (enemy_approach, enemy_approach_left, enemy_approach_right))
    .run()
}
...
fn spawn_background(
    ...
    commands
        .spawn(SpriteBundle{
            sprite: Sprite { color: Color::rgb(0.1, 0.1, 0.8), custom_size: Some(Vec2::new(5000., 5000.)), ..default()},
...
fn enemy_approach_left(
    mut left_hand: Query<&mut Transform, With<LeftHand>>,
){
    for mut left_transform in left_hand.iter_mut(){
        ...
        left_transform.translation.y = ENEMY_DISPLACEMENT_FROM_CENTER + (HAND_DISPLACEMENT * left_transform.scale.y);
        left_transform.translation.x = LEFT_HAND_DISTANCE * left_transform.scale.x;
    }
}

fn enemy_approach_right(
    mut right_hand: Query<&mut Transform, With<RightHand>>,
){
    for mut right_transform in right_hand.iter_mut(){
        ...
        right_transform.translation.y = ENEMY_DISPLACEMENT_FROM_CENTER + (HAND_DISPLACEMENT * right_transform.scale.y);
